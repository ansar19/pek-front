        ...
        <a class="navbar-brand w-100 mr-0" href="#" style="line-height: 25px;">
          <div class="d-table m-auto">
            <img id="main-logo" class="d-inline-block align-top mr-1" style="max-width: 45px"
              src="@/assets/images/ecomarine-rus-1357x889.png" alt="EcoMarine Waste Management" />
      ...
      <div v-if="items" v-for="(nav, navIdx) in items" :key="navIdx">
        ...
        <d-nav v-if="typeof nav.items !== 'undefined' && nav.items.length" class="nav--no-borders flex-column">
          <li v-for="(item, navItemIdx) in nav.items" :key="navItemIdx" class="nav-item dropdown">
            <d-link :class="['nav-link', item.items && item.items.length ? 'dropdown-toggle' : '']" :to="item.to"
            ...
            <d-collapse v-if="item.items && item.items.length" :id="`snc-${navIdx}-${navItemIdx}`"
