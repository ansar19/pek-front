<template>
  <div>
    <div>
      ...
      <button name="button" @click="fitPolyline">
        Fit map to polyline
      </button><br><br>
      ...
      <hr>
      <h5>Точки радиационного мониторинга</h5>
      ...
      <table class="meta-table"> 
        ...
        <tr v-for="(item, index) in radiationMonitoringPoints" :key="index">
          <td>{{ '№ ' + (index + 1) }}</td>
          <td>
          ...
          <td>
              {{ item.radiationMonitoringPoint }}
            <!-- <input v-model="item.radiationMonitoringPoint" type="text" > -->
          </td>
    ...
    <l-map :zoom.sync="zoom" :options="mapOptions" :center="center" :bounds="bounds" :min-zoom="minZoom"
      ...
      <l-marker v-for="marker in radiationMonitoringPoints" :key="marker.id" :visible="marker.visible" :draggable="marker.draggable"
        :lat-lng.sync="marker.position" :icon="marker.icon" @click="alert(marker)">
        <l-popup :content="marker.radiationMonitoringPoint" />
        <l-tooltip :content="marker.radiationMonitoringPoint" />
      </l-marker>
      ...
      <l-layer-group v-for="item in stuff" :key="item.id" :visible.sync="item.visible" layer-type="overlay"
        name="Layer 1">
...
export default {
  ...
  data() {
    ...
    radiationMonitoringPoints: [{
      ...
      bounds: latLngBounds(
        { lat: 49.781264058178344, lng: 80.3323681687735 },
        { lat: 49.75110537454436, lng: 82.92169544458676 }
      ),
    };
  },
  methods: {
    ...
    fitPolyline: function() {
      const bounds = latLngBounds(radiationMonitoringPoints.map(o => o.position));
      this.bounds = bounds;
    },
  },
};
